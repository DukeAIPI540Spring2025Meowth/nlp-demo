...
def main():
    ...
    dataset = load_dataset("thu-coai/esconv", split="train", trust_remote_code=True)

    # Save as JSON Lines format
    with open("esconv/train.json", "w", encoding="utf-8") as f:
    ...
    print(f"Saved {len(dataset)} lines to esconv/train.json")
